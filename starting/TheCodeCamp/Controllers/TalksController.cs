        ...
        public async Task<IHttpActionResult> Post(string moniker, TalkModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    //recuperamos el camp donde vamos a insertar la talk usando el moniker.
                    var camp = await _repository.GetCampAsync(moniker);
                    if (camp != null)
                    {
                        Talk talk = _mapper.Map<Talk>(model);       // Mapeamos el objeto Talk con los campos recibidos del modelo
                        talk.Camp = camp;                           // Agregamos el camp, recuperado para mantener la relación.

                        //Map speaker si es necesario
                        ...
                        {
                            // Devolvemos utiliznado el CreatedAtRoute
        ...
        public async Task<IHttpActionResult> Put(string moniker, int talkId, TalkModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // Por configuración del mapper se va a ingonrar al Spekar
                    _mapper.Map(model, talk);

                    // Si es neceario actualizar al spekar hemos de hacerlo manualmente.
